...
if (!window._pipScriptLoaded) {
window._pipScriptLoaded = true;
...
async function openPiPWindow() {
  try {
    ...
    if (isGeminiHost) {
      ...
      if (!isInConversation) {
        throw new Error(
          "Vui lòng gửi 1 tin nhắn để mở một cuộc trò chuyện mới trước khi bật Always On Top."
        );
      }
      console.log("----> ✓ Đang ở trong conversation (Grok):", urlPath);
    } else {
      console.log("----> ✓ Bỏ qua kiểm tra conversation cho phần non-Gemini host khác");
    }

    const isGrok = urlObj.hostname.includes("grok.com");
    ...
    pipWindow.document.body.style.background = "rgb(5,5,5)";
    ...
    if (isGrok) {
      // ==================================================
      // GROK MODE: Chỉ hiện progress window, KHÔNG move DOM
      // (moving DOM crashes React reconciler khi Grok routing)
      // ==================================================
      console.log("----> [Grok Mode] Mở PiP chỉ để hiển thị tiến độ - KHÔNG move DOM");

      // Tạo Progress Display ngay trong PiP
      createProgressDisplay(pipWindow);

      console.log("----> ✓ PiP window Grok đã sẵn sàng (Progress-only mode)!");
      console.log("----> 💡 Grok vẫn chạy trong tab gốc, PiP chỉ hiển thị tiến độ");

      await new Promise((r) => setTimeout(r, 300));

      // Cleanup khi đóng PiP
      ...
      return; // Kết thúc sớm - không cần các bước Gemini
    }

    // ==================================================
    // GEMINI MODE: Copy styles + Move DOM (chiến lược cũ)
    // ==================================================
    console.log("----> 3. Đang sao chép giao diện (Gemini mode)...");

    // Tìm container Gemini
    geminiContainer =
    ...
    console.log(
      "----> 💡 Giao diện giống 100% bản gốc (Dark mode, fonts, colors)",
    );
  ...
  const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      mutation.addedNodes.forEach((node) => {
        ...
        if (isOverlay) {
          console.log(
            "----> 🔄 Phát hiện dropdown/modal, đang redirect vào PiP...",
          );
...
function createProgressDisplay(win) {
  ...
  console.log(
    "----> ✓ Progress Display đã được tạo (HTML injection, có thể ẩn/hiện)",
  );
}
...
function updatePiPProgress(data) {
  ...
  try {
    ...
    console.log(
      `----> ✓ Progress updated in PiP: ${completed}/${total} - ${currentChapter}`,
    );
  } catch (error) {
...
} // End of window._pipScriptLoaded check
