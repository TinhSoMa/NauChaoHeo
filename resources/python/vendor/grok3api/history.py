...
class History:
    ...
    def set_main_system_prompt(self, text: str):
        try:
            ...
            logger.error(f"Lỗi trong set_main_system_prompt: {e}")
    ...
    def add_message(self, history_id: str,
        ...
        try:
            ...
            logger.error(f"Lỗi trong add_message: {e}")
    ...
    def get_history(self, history_id: str) -> str:
        try:
            ...
            logger.error(f"Lỗi trong get_history: {e}")
    ...
    def del_history_by_id(self, history_id: str) -> bool:
        ...
        try:
            ...
            logger.debug(f"Lịch sử với ID {history_id} đã được xóa.")
            return True
        except Exception as e:
            logger.error(f"Lỗi trong delete_history: {e}")
            return False
    ...
    def to_file(self):
        try:
            ...
            logger.error(f"Lỗi trong save_history: {e}")

    async def async_to_file(self):
        """Lưu dữ liệu bất đồng bộ vào tệp JSON."""
        try:
            data = {
                "chat_histories": self._chat_histories,
                "system_prompts": self._system_prompts,
            ...
            logger.error(f"Lỗi trong to_file: {e}")
    ...
    def from_file(self):
        try:
            ...
            logger.debug("Lỗi load_history: Không tìm thấy tệp.")
        except Exception as e:
            logger.error(f"Lỗi trong load_history: {e}")


def encode_image(image: Union[str, BytesIO]) -> Optional[tuple[str, str]]:
    """Mã hóa ảnh sang base64 và xác định loại ảnh."""
    try:
        if isinstance(image, str):
            ...
            raise ValueError("Ảnh phải là đường dẫn tệp hoặc đối tượng BytesIO")
        ...
        logger.error(f"Lỗi trong encode_image: Không tìm thấy tệp {image}.")
        return None
    except Exception as e:
        logger.error(f"Lỗi trong encode_image: {e}")
        return None
