...
export function createDeepSeekProvider(): AIProvider {
  return {
    transport: 'deepseek',
    async call({ prompt, systemPrompt, model, signal, debugSaveDir, batchIndex }: AIProviderParams): Promise<AIProviderResult> {
      const response = await callDeepSeekChat(prompt, { systemPrompt, model, signal, debugSaveDir, batchIndex });
